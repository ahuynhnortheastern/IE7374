...
Come along as we develop a Retrieval-Augmented Generation (RAG) system that transforms tabular hospital datasets into easy-to-understand summaries that even cavemen can understand. We will be using three datasets from the Center of Medicare and Medicaid Services (CMS): Patient Survey (HCAHPS), Healthcare-Associated Infections (HAI), and Timely & Effective Care. These metrics are critical in understanding the quality, performance, and level of patient safety for hospitals. 

We start by cleaning and preprocessing the publicly available data and follow by generating statistical insight for each facility. These insights are then converted into structured documents with accompanying metadata in order to be indexed within a vector. Similar to referencing research papers and sources in a college-level essay, we will leverage large language model to retrieve important information therefore ensuring credibility.
